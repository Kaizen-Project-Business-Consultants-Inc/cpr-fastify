...
test.describe.serial('Workflow: availability -> course request -> instructor assignment', () => {
  const date = farFutureDate(45);
  const location = `${TEST_MARKER} (location)`;
  const notes = TEST_MARKER;
  ...
  test('instructor marks a future date available', async ({ browser }) => {
    ...
    expect(res.ok(), `POST /instructor/availability -> ${res.status()}: ${await res.text()}`).toBeTruthy();
...
test.describe.serial('Workflow: vendor invoice submit -> approve -> pay', () => {
  ...
  test('vendor uploads an invoice', async ({ browser }) => {
    ...
    await pg.getByLabel('Rate').fill('10.00');
    await pg.getByLabel('Subtotal').fill('10.00');
    await pg.getByLabel('Total', { exact: true }).fill('10.00');
    ...
    await pg.getByRole('button', { name: 'Upload Invoice' }).click();
  ...
  test('vendor submits the invoice to admin', async ({ browser }) => {
    ...
    const row = pg.getByText(invoiceNumber, { exact: false }).first();
    await expect(row, 'the just-uploaded invoice should be listed').toBeVisible({ timeout: 30000 });
  ...
  test('admin forwards the invoice to accounting', async ({ browser }) => {
    ...
    const row = pg.getByText(invoiceNumber, { exact: false }).first();
    await expect(row, 'the vendor invoice should be visible to admin').toBeVisible({ timeout: 30000 });
    await pg.getByRole('button', { name: `View invoice ${invoiceNumber}` }).click();
  ...
  test('accountant records payment on the vendor invoice', async ({ browser }) => {
    ...
    const row = pg.getByText(invoiceNumber, { exact: false }).first();
    await expect(row, 'the approved vendor invoice should be visible to accounting').toBeVisible({
      timeout: 30000,
    });
    await pg.getByRole('button', { name: `View invoice ${invoiceNumber}` }).click();
